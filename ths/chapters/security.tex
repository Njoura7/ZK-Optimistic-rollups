...
We assume the adversary controls the sequencer process but not the Layer~1 consensus mechanism. The L1 chain is assumed to be secure under standard proof-of-stake assumptions (i.e., at least two-thirds of validators are honest). The following sections analyze how each architecture responds to these threats, with references to the empirical results from Chapter~\ref{ch:evaluation} where applicable.
...
\textbf{Cost of security.} The ZK~Rollup's security comes at a measurable cost: approximately 2.6$\times$ higher L1 gas consumption (Chapter~\ref{ch:evaluation}). For applications where the security premium is justified by the value at risk, this cost is acceptable. For lower-value applications, the Optimistic model's cheaper security may be sufficient.
