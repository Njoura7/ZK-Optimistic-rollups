...
\textbf{Gas costs.}
In the Hardhat test environment, the ZK~Rollup consumed approximately 2.6$\times$ more total L1 gas than the Optimistic~Rollup for the same transaction volume, with a per-transaction cost roughly 3$\times$ higher. These are relative comparisons on a local node; absolute mainnet figures will differ. The cost difference reflects the price of cryptographic finality.
...
\begin{table}[h]
    ...
    \begin{tabular}{c|p{4.5cm}|p{5.5cm}}
        ...
        \hline
        3 & Where do the real costs land---computation, gas, or time? & ZK is faster but uses $\sim$2.6$\times$ more L1 gas (Hardhat estimate). Optimistic is cheaper but slower. Proof generation adds $\sim$95ms P95 latency per batch, not a bottleneck at this scale. \\
        \hline
